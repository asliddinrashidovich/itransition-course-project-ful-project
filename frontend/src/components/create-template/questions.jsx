...
function AllQuestions() {
  ...
  const handleChangeQuestionType = async (val, questionId) => {
    await axios
      .patch(
        `${API}/api/templates/questions/${questionId}/type`,
        { type: val },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      .then(() => setRefreshQuestions((prev) => !prev))
      .catch((err) => toast.error(err.response?.data?.message || "Something went wrong"));
  };

  const handleAddQuestion = async () => {
    await axios
      .post(`${API}/api/templates/${id}/questions`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => setRefreshQuestions((prev) => !prev))
      .catch((err) => toast.error(err.response?.data?.message || "Something went wrong"));
  ...
  const fetchLatestTemplate = async () => {
    ...
    // title'larni statega joylaymiz
    const titles = {};
    res.data.questions.forEach(q => {
      titles[q.id] = q.title || "Untitled Question";
    });
  ...
  const { data: LatestTemplate } = useQuery({
    ...
    queryFn: fetchLatestTemplate,
  });

  // Title o'zgarishini yuboruvchi debounced PATCH funksiyasi
  const saveQuestionTitle = debounce(async (id, title) => {
    try {
      await axios.put(`${API}/api/templates/questions/${id}/title`, { title }, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } catch (err) {
  ...
  useEffect(() => {
    LatestTemplate?.questions?.forEach((q) => {
      ...
      if (input && span) {
        ...
        input.style.width = span.offsetWidth + 6 + "px";
      }
    });
          ...
          <div key={item.id} className="w-full bg-[#fff] border-l-[7px] border-[#0048ff] rounded-[10px] py-[30px] px-[20px]">
            <div className="flex items-center justify-between">
              <div className="relative inline-block">
                ...
                >
                  {questionTitles[item.id] || "Untitled Question"}
                </span>
                <input
                  ...
                  className="outline-none text-[18px] font-[600] border-[1px] border-[#999] "
                />
              </div>

              <select
                value={item.type}
                onChange={(e) => handleChangeQuestionType(e.target.value, item.id)}
                className="border-[1px] border-[#999] text-[#999] rounded-[4px] cursor-pointer px-[10px] py-[5px]"
              >
            ...
            {item.type === "short_text" && (
              <p className="pt-[20px] text-[#999] pb-[10px] border-b-[1px] border-[#999] w-[50%]">Short answer text</p>
            )}
            {item.type === "paragraph" && (
              <p className="pt-[20px] text-[#999] pb-[10px] border-b-[1px] border-[#999] w-[65%]">Long answer text</p>
            )}
            {item.type === "number" && (
              <p className="pt-[20px] text-[#999] pb-[10px] border-b-[1px] border-[#999] w-[25%]">Number answer</p>
            )}
            {item.type === "checkbox" && (
              <div className="pt-[20px] text-[#999] pb-[10px] border-b-[1px] border-[#999] w-[30%]">

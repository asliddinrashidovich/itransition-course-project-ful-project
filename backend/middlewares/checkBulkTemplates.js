...
const checkBulkTemplateOwner = async (req, res, next) => {
  ...
  try {
    ...
    const unauthorizedTemplates = templates.filter((template) => template.authorId != userId && !isAdmin);
    if (unauthorizedTemplates.length > 0) {
      return res.status(403).json({ message: 'You are not allowed to delete some of these templates.', unauthorizedIds: unauthorizedTemplates.map(t => t.id),});
    }
